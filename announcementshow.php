...
$query_Update = "UPDATE news_album SET album_hits=album_hits+1 WHERE album_id = ".$_GET["album_id"];
...
mysql_select_db($database_connSQL, $connSQL);
$query_RecAlbum = sprintf("SELECT * FROM news_album WHERE album_id = %s", GetSQLValueString($colname_RecAlbum, "int"));
...
mysql_select_db($database_connSQL, $connSQL);
$query_RecPhoto = sprintf("SELECT * FROM news_photo WHERE album_id = %s ORDER BY ap_date DESC", GetSQLValueString($colname_RecPhoto, "int"));
