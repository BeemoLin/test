...
$query_Update = "UPDATE rules SET rules_hits=rules_hits+1 WHERE rules_id = ".$_GET["rules_id"];
...
if (isset($_GET['rules_id'])) {
  $colname_RecAlbum = $_GET['rules_id'];
}
mysql_select_db($database_connSQL, $connSQL);
$query_RecAlbum = sprintf("SELECT * FROM rules WHERE rules_id = %s", GetSQLValueString($colname_RecAlbum, "int"));
...
$colname_RecPhoto = "-1";
if (isset($_GET['rules_id'])) {
  $colname_RecPhoto = $_GET['rules_id'];
}
mysql_select_db($database_connSQL, $connSQL);
$query_RecPhoto = sprintf("SELECT * FROM rules_photo WHERE rules_id = %s ORDER BY ap_date DESC", GetSQLValueString($colname_RecPhoto, "int"));
