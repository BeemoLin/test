...
class sam_pages_class{
  ...
  function getFirstpage3($life_name, $page_name = null){                                  //取得第一頁 (生活資訊特製)
    $page="";
    //$page.='<a href="?page=1" >第一頁</a>' ;
    $page.='<a href="#" onclick="post_to_url(\''.$this->page_name.'\', {\'action_mode\':\''.$this->action_mode.'\',\'life_name\':\''.$life_name.'\',\'page\':\'1\'})">第一頁</a>'."&nbsp;" ;
    return $page;
  }
  
  function getEndpage3($life_name, $page_name = null){                                    //取得最終頁 (生活資訊特製)
    $page="";
    $page.='<a href="#" onclick="post_to_url(\''.$this->page_name.'\', {\'action_mode\':\''.$this->action_mode.'\',\'life_name\':\''.$life_name.'\',\'page\':\''.$this->total_pages.'\'})">第終頁</a>' ;
    return $page;
  }
  
  function getListpage3($life_name, $view_page,$page_name = null){               					//取得頁數列表 (生活資訊特製)
    $page='';
    $this->view_page = $view_page;
    if($this->now_page-$this->view_page>1){
      $page.="...&nbsp;";
    ...
    for($i=$this->now_page-$this->view_page;$i<$this->now_page;$i++){
      if($i>0){
        $page.='<a href="#" onclick="post_to_url(\''.$this->page_name.'\', {\'action_mode\':\''.$this->action_mode.'\',\'life_name\':\''.$life_name.'\',\'page\':\''.$i.'\'})">'.$i.'</a>'."&nbsp;" ;
      }
    }
    $page.='<a href="#" onclick="post_to_url(\''.$this->page_name.'\', {\'action_mode\':\''.$this->action_mode.'\',\'life_name\':\''.$life_name.'\',\'page\':\''.$this->now_page.'\'})" style="font-weight:bold;color:red;">'.$this->now_page.'</a>'."&nbsp;" ;
    for($i=$this->now_page+1;$i<=$this->now_page+$this->view_page;$i++){
      if($i<=$this->total_pages){
        $page.='<a href="#" onclick="post_to_url(\''.$this->page_name.'\', {\'action_mode\':\''.$this->action_mode.'\',\'life_name\':\''.$life_name.'\',\'page\':\''.$i.'\'})">'.$i.'</a>'."&nbsp;" ;
      }
