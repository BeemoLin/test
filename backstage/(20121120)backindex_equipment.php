...
include(BCLASS.'/head.php'); //包在CLASS裡面 裡面有post_to_url參數重要因為轉址用
 ...
 // echo "1111".$_POST['insertmode'];
  if(isset($_POST['insertmode']))//使用html元件 input type=hidden好用可以用javascript去將mode分為1 insert 2 delete 3 update.....
  {
    ...
    */
    ...
    $list_datetime=$list_date." ".$list_time;
    $save_datetime="NOW()";//使用MYSQL的NOW函數
    $m_id=$_POST['set_m_id'];
    $list_using_number="1";
    $list_remarks=$_POST['list_remarks'];
    $list_disable='0'; //問一下本圓這個欄位的意義
    //先取值 資料庫攔位與HTML物件名稱與變數名稱都統一一樣
    //echo $equipment_id;
    
    $data_function->setDb("equipment_reservation_list");
    $expression = "`equipment_id`='".$equipment_id."',`list_date` = '".$list_date."',`list_time` = '".$list_time."',`list_datetime` = '".$list_datetime."',`save_datetime` = ".$save_datetime.",`m_id` = '".$m_id."',`list_using_number` = '".$list_using_number."',`list_remarks` = '".$list_remarks."',`list_disable` = '".$list_disable."'";//,`m_id` = '".$pic_subject."'"
    $data_function->insert($expression);
