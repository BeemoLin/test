    ...
    */
    ...
    $("#bGenImage").click(function () 
    {
      ...
      //$("#admin_sign_code")[0].value=$canvas2[0].toDataURL();
       ...
       document.getElementById('action_mode').value="finish_check";
       document.getElementById("sendorupdate").value = "send";
       
       
       $('#form').submit();
       return;
       
     // alert(document.getElementById("sendorupdate").value);
//----------------------------兩著都簽名才能信件發放-------------------------------------------------------------------------------------------
    ...
    //20131116 增加資料修正:單純更改資料庫內容,不發送信件
    $("#bUpdateDB").click(function () 
    {
  // var count = document.elements["arr[][]"];
  // alert(count);
  // return;
//--------------------------------------------------------------------------------------------   
      
       //alert(document.getElementById("sendorupdate").value);
       //document.getElementById("cou").value = "200";
      /*
      alert(document.getElementById("letter_category").value);
      alert(document.getElementById("letter_alt").value);
      alert(document.getElementById("receives_name").value);// = "200";
      alert(document.getElementById("m_username").value);
      alert(document.getElementById("letters_number").value);// = "200";
      */
      if(document.getElementById("letter_category").value=="")
      {
        alert("未選擇信件種類");
        return;
      }
      if(document.getElementById("letter_alt").value=="")
      {
        alert("未選擇公司名稱");
        return;
      }
      if(document.getElementById("receives_name").value=="")
      {
        alert("未填寫收件者姓名");
        return;
      }
      ...
      if(document.getElementById("letters_number").value=="")
      {
        alert("未填寫函件編號 ");
        return;
      }
      ...
      //return;
      document.getElementById("sendorupdate").value = "updatedb";
...
function AddPic(){
   /* var my_title = document.getElementById("<?php echo $input_title;?>");
    ...
    return false;*/
    document.getElementById('action_mode').value="addpic";
...
function chang_pic_subject(path, input_id, pic_id, pic_subject_name){
  ...
  post_to_url(path,{'action_mode':'update_image', 'maintainlog_uid':input_id, 'uid':pic_id, 'check_picurl_subject':pic_subject});
}
</script>
...
<div id="main">
  <div id="main_fix_table">
    <form enctype="multipart/form-data" name="form" id="form" method="post" action="backindex_maintlog.php">
      ...
      <input type="hidden" name="action_mode" id="action_mode" value="<?=$action_mode?>"><!--control:fix-->
      <input type="hidden" name="uid" id="uid" value="<?=$uid?>"><!--被選擇的index-->
      ...
      <input type="hidden" id='sendorupdate' name='sendorupdate' />
...
<!--貨運公司 ：<select name="arr[<?php echo $no; ?>][letter_alt]" id="arr[<?php echo $no; ?>][letter_alt]">-->
      ...
      <div style="position: relative; border: 0px solid; width: 860px; height: 550px;">
        <p class="heading"><?php echo $img_title;?></p>
         ...
         <!--class="show" style="clear:both;"-->
          <?php  if($no>0){?>
           <div style="border: 0px solid; width: 160px; height: 170px;"></div>
           <?php }?>
          <?php
          $show_no = 1;
      ...
      ?>
        ...
        <div style="float: left">
          <input type="button" class="btn btn-primary" name="addpic" id="addpic" onClick="AddPic();" value="新增照片" />
          <input type="button" class="btn btn-success" id="bGenImage" value="確定送出"><!--檢查winform送入後端的攔位內容條件-->
          <!--<input type="reset" value="清除重填">-->
          <?php if(3<2){?>
          <input type="button" class="btn btn-warning" id="bUpdateDB" value="資料修正">
          <?php }?>
          <input type="button" class="btn btn-danger" id="back" value="回上一頁" onClick="post_to_url('backindex_maintlog.php', {'action_mode':'index','equipment_id':'<?=$main_id?>'})" />
          <input type="hidden" name="pen" id="cou" value="100">
        </div>
      </div>
