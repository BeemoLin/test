...
$query_Update = "UPDATE living SET living_hits=living_hits+1 WHERE living_id = ".$_GET["living_id"];
...
$colname_RecLiving = "-1";
if (isset($_GET['living_id'])) {
  $colname_RecLiving = $_GET['living_id'];
}
mysql_select_db($database_connSQL, $connSQL);
$query_RecLiving = sprintf("SELECT * FROM living WHERE living_id = %s", GetSQLValueString($colname_RecLiving, "int"));
$RecLiving = mysql_query($query_RecLiving, $connSQL) or die(mysql_error());
$row_RecLiving = mysql_fetch_assoc($RecLiving);
$totalRows_RecLiving = mysql_num_rows($RecLiving);

$colname_RecPhoto2 = "-1";
if (isset($_GET['living_id'])) {
  $colname_RecPhoto2 = $_GET['living_id'];
}
mysql_select_db($database_connSQL, $connSQL);
$query_RecPhoto2 = sprintf("SELECT * FROM living_photo2 WHERE living_id = %s ORDER BY app_date ASC", GetSQLValueString($colname_RecPhoto2, "int"));
