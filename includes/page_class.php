...
class sam_pages_class{
  ...
  function setLink($link_type){                             //設定連接型態(未使用)
  ...
  function getFirstpage(){                                  //取得第一頁
    return '<a href="?page=1">第一頁</a>' ;
  }
  
  function getEndpage(){                                    //取得最終頁
    return '<a href="?page='.$this->total_pages.'">最終頁</a>' ;
  }
  
  function getListpage($now_page,$view_page){              //取得頁數列表
    if($now_page-$view_page>1){
      $data = "... ";
    }
    for($i=$now_page-$view_page;$i<$now_page;$i++){
      if($i>0){
        $data .= '<a href="?page='.$i.'">'.$i.'</a> ' ;
      }
    }
    $data .= '<a href="?page='.$now_page.'" style="font-size: 18px;font-weight:bold;color:red;">'.$now_page.'</a> ' ;
    for($i=$now_page+1;$i<=$now_page+$view_page;$i++){
      if($i<=$this->total_pages){
        $data .= '<a href="?page='.$i.'">'.$i.'</a> ' ;
      }
    }
    if($now_page+$view_page<$this->total_pages){
      $data .= "... ";
    }
		return $data;
  }
  ...
  function getFirstpage3(){                                  //取得第一頁
    return '<a href="'.$this->page_name.'&page=1">第一頁</a>' ;
  }
  
  function getEndpage3(){                                    //取得最終頁
    return '<a href="'.$this->page_name.'&page='.$this->total_pages.'">最終頁</a>' ;
  }
  
  function getListpage3($now_page,$view_page){              //取得頁數列表
    if($now_page-$view_page>1){
      $data = "... ";
    }
    for($i=$now_page-$view_page;$i<$now_page;$i++){
      if($i>0){
        $data .= '<a href="'.$this->page_name.'&page='.$i.'">'.$i.'</a> ' ;
      }
    }
    $data .= '<a href="'.$this->page_name.'&page='.$now_page.'" style="font-size: 18px;font-weight:bold;color:red;">'.$now_page.'</a> ' ;
    for($i=$now_page+1;$i<=$now_page+$view_page;$i++){
      if($i<=$this->total_pages){
        $data .= '<a href="'.$this->page_name.'&page='.$i.'">'.$i.'</a> ' ;
      }
